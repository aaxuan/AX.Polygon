    ...
    var miniMenu = {
        ...
        renderLeftMenu: function (leftMenus, options) {
            ...
            var leftMenusHtml = me.each(leftMenus || [], function (idx, leftMenu) { // 左侧菜单遍历
                ...
                var leftMenuHtml = me.compileMenu({
                    href: leftMenu.Menu.Url,
                    target: leftMenu.Menu.Target,
                    childOpenClass: options.childOpenClass,
                    icon: leftMenu.Menu.Icon,
                    title: leftMenu.Menu.Name,
                    children: children,
                    className: '',
                });
                return leftMenuHtml;
        ...
        renderChildrenMenu: function (menuList, options) {
            ...
            var html = this.each(menuList, function (idx, menu) {
                if (menu.Child && menu.child.length) {
                    menu.Menu.children = me.renderChildrenMenu(menu.Child, { childOpenClass: options.childOpenClass || '' });
                }
        ...
        renderMultiModule: function (menuList, menuChildOpen) {
            ...
            var headerMenuHtml = this.each(menuList, function (index, val) { //顶部菜单渲染
                ...
                topMenuItemHtml = miniMenu.compileMenu({
                ...
                leftMenuHtml += miniMenu.renderLeftMenu(val.Child, {
                    parentMenuId: menu,
                    childOpenClass: childOpenClass,
                    leftMenuCheckDefault: leftMenuCheckDefault
                });

                headerMobileMenuHtml += miniMenu.compileMenu({ id: id, menu: menu, id: id, icon: val.Menu.icon, title: val.Menu.Name, }, true);
